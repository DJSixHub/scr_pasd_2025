...
def load_and_preprocess_data(file_paths, target_col=None, test_size=0.2, random_state=42, scale=False):
    """
    Load and preprocess multiple datasets in parallel using Ray.
    ...
    try:
        logger.info(f"Loading and preprocessing {len(file_paths)} datasets")
        
        # Load datasets in parallel
        dataset_refs = [load_dataset.remote(file_path) for file_path in file_paths]
        datasets = ray.get(dataset_refs)
        ...
        processed_datasets = ray.get(preprocessing_refs)
